  ...
  if (!client) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <p className="text-[#9B9B90] text-lg">No client data available. Complete onboarding to get started.</p>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-7">
        <div>
          <h1 className="text-[22px] font-semibold text-[#111110]">Reports</h1>
          <p className="text-[13px] text-[#6B6B60]">
            {dateRange.from} to {dateRange.to} · {platformSummaries.map(p => p.platform).join(', ') || 'No platforms connected'}
          </p>
        </div>
        <DateRangePicker />
      </div>
      ...
      <div className="grid grid-cols-[2fr_1fr] gap-4 mb-6">
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-[15px] font-semibold text-[#111110]">Revenue vs. Spend</h3>
              <p className="text-xs text-[#9B9B90]">Daily, all platforms blended</p>
            </div>
            <div className="flex gap-4">
              <span className="flex items-center gap-1.5 text-xs text-[#9B9B90]"><span className="w-2 h-2 rounded-full bg-[#2D7A4F]" /> Revenue</span>
              <span className="flex items-center gap-1.5 text-xs text-[#9B9B90]"><span className="w-2 h-2 rounded-full bg-[#4285F4]" /> Spend</span>
            </div>
          </div>
        ...
        <Card className="p-6">
          <div className="mb-4">
            <h3 className="text-[15px] font-semibold text-[#111110]">Budget Pacing</h3>
            <p className="text-xs text-[#9B9B90]">{new Date().toLocaleString('en-US', { month: 'long', year: 'numeric' })} · {formatCurrencyExact(budget)} target</p>
          </div>
          <BudgetPacing
            items={[
      ...
      <div className="grid grid-cols-[2fr_1fr] gap-4 mb-6">
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-[15px] font-semibold text-[#111110]">ROAS by Platform</h3>
              <p className="text-xs text-[#9B9B90]">Daily trend · Target: {formatRoas(client.roas_target || 3)}</p>
            </div>
            <div className="flex gap-4">
              <span className="flex items-center gap-1.5 text-xs text-[#9B9B90]"><span className="w-2 h-2 rounded-full bg-[#4285F4]" /> Meta</span>
              <span className="flex items-center gap-1.5 text-xs text-[#9B9B90]"><span className="w-2 h-2 rounded-full bg-[#34A853]" /> Google</span>
              <span className="flex items-center gap-1.5 text-xs text-[#9B9B90]"><span className="w-2 h-2 rounded-full bg-gray-300" /> Target</span>
            </div>
          </div>
        ...
        <Card className="p-6">
          <div className="mb-4">
            <h3 className="text-[15px] font-semibold text-[#111110]">Revenue by Channel</h3>
            <p className="text-xs text-[#9B9B90]">MTD</p>
          </div>
