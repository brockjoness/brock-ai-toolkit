#!/usr/bin/env python3
"""
Analyze face directions in your reference photos using MediaPipe.

This script:
1. Loads photos from reference-photos/raw/
2. Analyzes face pose (yaw/pitch) using MediaPipe
3. Renames and moves to reference-photos/ with direction in filename

Naming convention: subject_yaw{L/R}{degrees}_pitch{U/D}{degrees}.jpg
Examples:
  - subject_yawL30_pitchU10.jpg  (looking 30 left, 10 up)
  - subject_yawR45_pitch0.jpg    (looking 45 right, level)
  - subject_yaw0_pitch0.jpg      (dead center)
...
# Paths
BASE_DIR = Path(__file__).parent.parent / "reference-photos"
...
def generate_filename(yaw: float, pitch: float, index: int = 0) -> str:
    ...
    base = f"subject_yaw{yaw_str}_pitch{pitch_str}"
    if index > 0:
        base += f"_{index}"
    return base + ".jpg"
...
def find_closest_reference(target_yaw: float, target_pitch: float) -> Path | None:
    ...
    refs = [
        ...
        if f.suffix.lower() in extensions and f.name.startswith("subject_")
    ]
